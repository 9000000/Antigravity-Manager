...
全面解决社区反馈的 5 项关键 Issue（**#3472、#3473、#3474、#3475、#3467**）以及多项深层链路缺陷，拔除了历史版本中错误的工具强行调用参数替换、动态消息插入错位、跨轮签名广播覆盖与思考块误降级抹除等一系列技术债务。
...
- **三大架构重构**：
  1. **签名与思考文本解耦法则**：签名与思考文本完全解耦互不约束。无 `tool_call` 一律哨兵占位，有 `tool_call` 严格以 `tool_id` 锚定；有思考文本填思考，无思考文本填 `"..."` 占位。
  2. **Responses 协议首位思考块强制保序与前缀绝对静止**：完美兼顾 Codex 普通进度汇报与 Gemini 3 原生思考块结构，历史前缀在每一轮对话中字节级绝对冻结。
  3. **一键清空思考块与全量 12 种语言国际化**：在设置界面滑动窗口栏新增一键清空思考块缓存与 SQLite 数据库功能，配有防误触二次确认与小字警示，**严禁触碰任何请求日志**。

实测在 4.7.7 版本下，Subagent 思考互不污染，并发会话缓存命中独立，Gemini 协议、Anthropic 协议、Responses 协议、OpenAI Chat 协议在多 Agent 并发场景下 **Prompt Cache 命中率均高达 85% ~ 98.5%**。

> 💡 **推荐操作**：升级新版本后，强烈建议在设置界面的“思考块双层滑动窗口”栏点击 **【清空思考块】** 按钮，彻底清空旧版残留的无 tool_id 畸形签名数据，以便完整享受最新的 SQLite 双层索引穿透与绝对冻结的前缀缓存设计。
